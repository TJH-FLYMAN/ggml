...
经典 Q 系列中，`QK4_0`、`QK5_1`、`QK8_0` 等宏的值表示一个 block 可还原出多少个浮点值，它们通常是 32。ggml保存的是缩放因子和偏移，而非零点值
- Q4_0、Q5_0：只有块级 scale d，属于对称式量化。而4和5分别代表一个量化值多少bit表示，
- Q4_1、Q5_1：保存 d 和 m（minimum/偏移量），属于仿射/非对称量化。

`QK_K` 在源码注释中定义为 super-block size，当前值为 256，一个量化块会处理256个原始浮点数。K 系列常在这 256 个值内再分成多个小块，共享超块级的 `d`/`dmin`，同时为小块保存压缩后的 `scales`。不过，该头文件也复用 `QK_K` 作为 TQ 和多数 IQ 类型的 256 元素 block 长度；使用 `QK_K` 并不意味着这个类型属于 K 系列。

d ：一个超级块（256个值）首先有一个统一的缩放因子 d
scales：已知基础缩放 d，256个值切分成多个子块（小块）。例如，对于 Q4_K，每个超块会被分成8个子块，每个子块包含32个值，每个子块单独计算一个“微调缩放因子” scales。
关键点：这个 scales 不是一个完整的浮点数，而是被压缩存储的（例如用一个4-bit的整数表示）
...
6. 最后到 [`src/ggml-cuda/`](../src/ggml-cuda/)、[`src/ggml-metal/`](../src/ggml-metal/) 等其他后端阅读特化内核。后端是否有某个特化实现，以当前 checkout 为准。

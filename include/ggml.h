...
// 判断指针宽度 stdint.h 中定义宏UINTPTR_MAX
#if UINTPTR_MAX == 0xFFFFFFFF 
    #define GGML_MEM_ALIGN 4  // 32位环境 4字节对齐
#else
    #define GGML_MEM_ALIGN 16   //64位环境 16字节对齐
#endif
...
extern "C" {
    ...
    struct ggml_init_params {
        // memory pool 内存池info
        size_t mem_size;   // bytes 大小
        void * mem_buffer; // if NULL, memory will be allocated internally , buffer起始地址
        bool   no_alloc;   // don't allocate memory for the tensor data .为张量数据分配内存,true = 不分配
    };
